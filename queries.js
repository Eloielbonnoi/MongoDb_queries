...
db.restaurants.find({ "grades.score": { $gte: 80, $lte: 100 } }); // db.restaurants.find({$and: [{"grades.score": {$gte: 80}}, {"grades.score": {$lte: 100}}]}).count();
db.restaurants.find({ "address.coord": { $lt: -95.754168 } });
db.restaurants.find({ $and: [{ "cuisine": { $ne: "American " } }, { "grades.score": { $gte: 70 } }, { "address.coord": { $lt: -65.754168 } }] }).count();
db.restaurants.find({ "cuisine": { $not: { $eq: "American " } }, "grades.score": { $gte: 70 }, "address.coord": { $lt: -65.754168 } }).pretty();
db.restaurants.find({ $and: [{ "cuisine": { $ne: "American " } }, { "grades.grade": { $eq: "A" } }, { "borough": { $ne: "Brooklyn" } }] }).sort({ "cuisine": -1 }).pretty();
...
db.restaurants.find({ $and: [{ "borough": "Bronx" }, { $or: [{ "cuisine": "American " }, { "cuisine": "Chinese" }] }] }).pretty();
...
db.restaurants.find({ $and: [{ $and: [{ "cuisine": { $eq: "Seafood" } }, { "cuisine": { $ne: { $in: ["American ", "Chinese"] } } }] }, { "name": { $regex: /^Wil/ } }] }, { "restaurant_id": 1, "name": 1, "borough": 1, "cuisine": 1, "_id": 0 }).pretty();
db.restaurants.find({ $and: [{ "grades.grade": "A" }, { "grades.score": 11 }, { "grades.date": ISODate("2014-08-11T00:00:00Z") }] }, { "restaurant_id": 1, "name": 1, "borough": 1, "cuisine": 1, "_id": 0 })
db.restaurants.find({ $and: [{ "grades.1.grade": "A" }, { "grades.1.score": 9 }, { "grades.1.date": ISODate("2014-08-11T00:00:00Z") }] }, { "restaurant_id": 1, "name": 1, "borough": 1, "cuisine": 1, "_id": 0 })
db.restaurants.find({ "address.coord.1": { $gte: 42, $lte: 52 } }, { "restaurant_id": 1, "name": 1, "address.coord": 1, "_id": 0 })
db.restaurants.find().sort({ "name": 1 })
db.restaurants.find().sort({ "name": -1 })
db.restaurants.find().sort({ "cuisine": 1, "borugh": -1 })
db.restaurants.find({ "address.street": { $exists: true } }, { "address.street": 1, "name": 1, "_id": 0 }).pretty()
db.restaurants.find({ "address.coord": { $type: ["double"] } }, { "address.coord": 1, "name": 1, "_id": 0 }).pretty()
db.restaurants.find({ "grades.score": { $mod: [7, 0] } }, { "restaurant_id": 1, "name": 1, "grades": 1, "_id": 0 }).pretty();
db.restaurants.find({ "name": { $regex: ".*mon.*" } }, { "name": 1, "borough": 1, "address.coord": 1, "_id": 0 }).pretty();
db.restaurants.find({ "name": { $regex: /^Mad/ } }, { "name": 1, "borough": 1, "address.coord": 1, "_id": 0 })
